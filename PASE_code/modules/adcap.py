...
class AP(nn.Module):
    def __init__(self, d_hidden):
        ...
        self.self_attn = nn.MultiheadAttention(d_hidden, num_heads=4, dropout=0.0, bias=True, batch_first=True)
    ...
    def forward(self, features, lengths):
        ...
        embed_features_ = (mask_features * embed_weights)

        # token-level
        # token_weights = [B x K x D]
        mask_features = mask_features.masked_fill(mask == 0, 0)
        token_weights = self.self_attn(embed_features_, mask_features, mask_features)[0]

        token_weights = self.linear(token_weights)
        ...
        # pool_features = token_features
        _  = None
        return pool_features, _