...
class EncoderImage_ProjFeature(nn.Module):
    def __init__(self, img_dim, emb_size, precomp_enc_type='basic', no_imgnorm=False):
        ...
        self.fc_1 = nn.Linear(self.img_dim, 512)

        self.mlp = MLP(img_dim, emb_size // 2, emb_size, 2)
        self.mlp_1 = MLP(img_dim, 512 // 2, 512, 2)
    ...
    def init_weights(self):
        ...
        r = np.sqrt(6.) / np.sqrt(self.fc_1.in_features +
                                  self.fc_1.out_features)
        self.fc_1.weight.data.uniform_(-r, r)
    ...
    def forward(self, img, img_lengths):
        ...
        fg_img_emb = self.fc_1(img)
        fg_img_emb = self.mlp_1(img) + fg_img_emb
        ...
        if not self.no_imgnorm:
            ...
            fg_img_emb = l2norm(fg_img_emb, dim=-1)


        return img_emb, fg_img_emb
