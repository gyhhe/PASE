...
class RegionLanguageDataset(data.Dataset):
    def __init__(self, data_path, data_split, vocab, tokenizer, args=None):
        ...
        # load entity_triples
        self.entity_triples = []
        with open(f'{self.data_path}/{data_split}_caps_{args.data_name}_entity_triples.txt', 'rb') as f:
            for line in f:
                self.entity_triples.append(line.decode().strip())

        # load subject
        self.subject = []
        with open(f'{self.data_path}/{data_split}_caps_{args.data_name}_subjects_triples.txt', 'rb') as f:
            for line in f:
                self.subject.append(line.decode().strip())

        # load relation
        self.relation = []
        with open(f'{self.data_path}/{data_split}_caps_{args.data_name}_relations_triples.txt', 'rb') as f:
            for line in f:
                self.relation.append(line.decode().strip())

        # load object
        self.object = []
        with open(f'{self.data_path}/{data_split}_caps_{args.data_name}_objects_triples.txt', 'rb') as f:
            for line in f:
                self.object.append(line.decode().strip())
    ...
    def __getitem__(self, index):
        ...
        # entity_triple
        entity_triple = self.entity_triples[index]
        if self.args.txt_enc_type == 'rnn':
            etokens = self.tokenizer(str(entity_triple).lower())
            entity_triple = aug_language(etokens, self.vocab,  self.data_split,
                                   self.args.txt_enc_type)
        elif self.args.txt_enc_type == 'bert':
            etokens = self.tokenizer.basic_tokenizer.tokenize(str(entity_triple).lower())
            entity_triple = aug_language(etokens, self.tokenizer,  self.data_split,
                                   self.args.txt_enc_type)

        # subject
        subject = self.subject[index]
        if self.args.txt_enc_type == 'rnn':
            sub_tokens = self.tokenizer(str(subject).lower())
            subject = aug_language(sub_tokens, self.vocab,  self.data_split,
                                   self.args.txt_enc_type)
        elif self.args.txt_enc_type == 'bert':
            sub_tokens = self.tokenizer.basic_tokenizer.tokenize(str(subject).lower())
            subject = aug_language(sub_tokens, self.tokenizer,  self.data_split,
                                   self.args.txt_enc_type)

        # relation
        relation = self.relation[index]
        if self.args.txt_enc_type == 'rnn':
            rel_tokens = self.tokenizer(str(relation).lower())
            relation = aug_language(rel_tokens, self.vocab,  self.data_split,
                                   self.args.txt_enc_type)
        elif self.args.txt_enc_type == 'bert':
            rel_tokens = self.tokenizer.basic_tokenizer.tokenize(str(relation).lower())
            relation = aug_language(rel_tokens, self.tokenizer,  self.data_split,
                                   self.args.txt_enc_type)

         # object
        object = self.object[index]
        if self.args.txt_enc_type == 'rnn':
            obj_tokens = self.tokenizer(str(object).lower())
            object = aug_language(obj_tokens, self.vocab, self.data_split,
                                    self.args.txt_enc_type)
        elif self.args.txt_enc_type == 'bert':
            obj_tokens = self.tokenizer.basic_tokenizer.tokenize(str(object).lower())
            object = aug_language(obj_tokens, self.tokenizer, self.data_split,
                                    self.args.txt_enc_type)

        # image = [region_num, img_dim]
        # caption = [seq_len, seq_dim]
        return image, caption, entity_triple, subject, relation, object, index, img_idx
...
def collate_sorted_fn(data):
    ...
    images, captions, entity_triples, subject, relation, object, ids, img_ids = zip(*data)
    ...
    # Merge entity_triples
    et_txt_lengths = torch.LongTensor([len(et) for et in entity_triples])
    et_txts = torch.zeros(len(entity_triples), max(et_txt_lengths)).long()
    for i, et in enumerate(entity_triples):
        end = et_txt_lengths[i]
        et_txts[i, :end] = et[:end]

    # Merge subject
    sub_txt_lengths = torch.LongTensor([len(sub) for sub in subject])
    sub_txts = torch.zeros(len(subject), max(sub_txt_lengths)).long()
    for i, sub in enumerate(subject):
        end = sub_txt_lengths[i]
        sub_txts[i, :end] = sub[:end]

    # Merge relation
    rel_txt_lengths = torch.LongTensor([len(rel) for rel in relation])
    rel_txts = torch.zeros(len(relation), max(rel_txt_lengths)).long()
    for i, rel in enumerate(relation):
        end = rel_txt_lengths[i]
        rel_txts[i, :end] = rel[:end]

    # Merge object
    obj_txt_lengths = torch.LongTensor([len(obj) for obj in object])
    obj_txts = torch.zeros(len(object), max(obj_txt_lengths)).long()
    for i, obj in enumerate(object):
        end = obj_txt_lengths[i]
        obj_txts[i, :end] = obj[:end]
    ...
    if len(images[0].shape) == 2:
        ...
        return imgs, img_lengths, txts, txt_lengths, et_txts, et_txt_lengths, sub_txts, rel_txts,  obj_txts, ids, img_ids
    else:
        imgs = torch.stack(images, 0)
        return imgs, txts, txt_lengths, et_txts, et_txt_lengths, sub_txts, rel_txts,  obj_txts, ids, img_ids
...
def collate_unsorted_fn(data):
    ...
    if len(images[0].shape) == 2:
        ...
        for i, img in enumerate(images):
            ...
            # end = min(et_txt_lengths[i], 13)  # 取实际长度和最大长度的最小值
            imgs[i, :end] = img[:end]
        img_lengths = torch.LongTensor(img_lengths)
