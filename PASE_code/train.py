...
from cross_modal_interaction import CrossModalInteractionLoss
...
def train(model, epoch, iterator, optimizer, loss_func, cross_modal_interaction_loss_func, logger, length, args):
    ...
    for step, data in enumerate(iterator):
        try:
            img, img_lengths, txt, txt_lengths, et_txts, et_txt_lengths, sub_txts, rel_txts,  obj_txts,  _, img_ids = data
        except:
            img, txt, txt_lengths, et_txts, et_txt_lengths, sub_txts, rel_txts,  obj_txts, _ = data
            img_lengths = None
        ...
        optimizer.zero_grad()
        fg_loss_func = ContrastiveLoss(args)
        # model prediction
        # img_emb = [batch_size, n_region, emb_size]
        # txt_emb = [batch_size, seq_len, emb_size]
        img_emb, txt_emb, fg_img_emb, fg_txt_emb, txt_lens = model(img, img_lengths, txt, txt_lengths, et_txts, et_txt_lengths, sub_txts, rel_txts,  obj_txts)
        fg_sims, L1 =  model.forward_sim(fg_img_emb, fg_txt_emb, txt_lens)
        loss, tmp_num_negs = loss_func(img_emb, txt_emb, img_ids)
        fg_loss= fg_loss_func(fg_sims)
        loss += fg_loss
        print('fg_loss', fg_loss.item())
        cross_modal_interaction_loss = cross_modal_interaction_loss_func(img_emb, txt_emb, img_ids)
        loss += cross_modal_interaction_loss
...
if __name__ == '__main__':
    ...
    #三元组损失
    loss_func = TripletLoss(args)
    #三元组
    cross_modal_interaction_loss_func = CrossModalInteractionLoss(args)
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        train(model, epoch, train_loader, optimizer, loss_func, cross_modal_interaction_loss_func, logger, len(train_loader), args)
        # save_checkpoint({
        #     'model': model.state_dict(),
        #     'rsum': 460,
        #     'args': args
        # }, True, filename='checkpoint.pth',
